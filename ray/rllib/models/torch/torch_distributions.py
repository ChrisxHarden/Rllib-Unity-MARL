...
class TorchMultiCategorical(Distribution):
    ...
    def __init__(
        ...
        super().__init__()
        self._cats = categoricals#The length of the catergorical list depends on action dim, [9,2] will lead to 2 dims
    ...
    def logp(self, value: torch.Tensor) -> TensorType:
        ...
        # with open("C:\\Users\\ezhao\\PycharmProjects\\MARL_Test\\testlogp.txt","w+") as logp_file:
        #     for item in self._cats:
        #         logp_file.write(str(item))
        #         logp_file.write("\nand the type is\n")
        #
        #         logp_file.write(str(type(item)))
        #
        #         logp_file.write("\n")
        #
        #
        #     logp_file.write(str(self._cats[0].logits.shape))
        #     logp_file.write("----------------------------------------\n")
        #
        #     for item in value:
        #         logp_file.write(str(item))
        ...
        #         logp_file.write("type of cat is:____________________________\n")
        #         logp_file.write(str(type(cat)))
        #         logp_file.write("\n")
        #         torch_cat=cat._get_torch_distribution(logits=cat.logits)
        #         logp_file.write("type of torch cat is:____________________________\n")
        #         logp_file.write(str(type(torch_cat)))
        #         logp_file.write("\n")
        #
        #
        #         logp_file.write("act is\n")
        #         logp_file.write(str(act))
        #
        #         logp_file.write("\n\n\n")
        #         logp_file.write("The result is\n")
        #         logps=torch.stack([torch_cat.log_prob(act) for cat, act in zip(self._cats, value)])
        #         logp_file.write(str(logps))
        #
    ...
    def from_logits(
        ...
        **kwargs,
        ...
        with open("C:\\Users\\ezhao\\PycharmProjects\\MARL_Test\\from_logits_log.txt", "w+") as file:
            file.write("input lens")
            file.write(str(len(input_lens)))
            for logits,temperature in zip(torch.split(logits, input_lens, dim=1), temperatures):
                file.write("logits------------------\n")
            ...
            file.write(str(zip(torch.split(logits, input_lens, dim=1), temperatures)))
