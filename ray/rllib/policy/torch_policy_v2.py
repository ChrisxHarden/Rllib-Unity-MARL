...
from ray.rllib.models.torch.torch_distributions import TorchDistribution,TorchCategorical,TorchMultiCategorical
...
class TorchPolicyV2(Policy):
    ...
    def _compute_action_helper(
        ...
        if isinstance(self.model, RLModule):
            ...
            # print("input_dict_check",input_dict)
            #SampleBatch(32: ['obs', 'new_obs', 'actions', 'prev_actions', 'rewards', 'prev_rewards', 'terminateds', 'truncateds', 'infos', 'eps_id', 'unroll_id', 'agent_index', 't'])
            ...
            if explore:
                ...
                #print("fwd_out_check",fwd_out) # This outputs the action distribution.
                ...
                if SampleBatch.ACTION_DIST_INPUTS in fwd_out:
                    ...
                    #<class 'ray.rllib.models.distributions.Distribution.get_partial_dist_cls.<locals>.DistributionPartial'>
                    action_dist = action_dist_class.from_logits(dist_inputs)
                    # if isinstance(action_dist,TorchDistributionWrapper):
                    #     with open(file_path, 'w+') as file:
                    #         # Do something with the file, like reading its contents
                    #         file.write("the actiondist is torchdistributionwrapper\n")




                    #action_dist <ray.rllib.models.distributions.Distribution.get_partial_dist_cls.<locals>.DistributionPartial object at 0x000001BAE52B7A60>
                ...
                # Otherwise, sample actions from the distribution.
                    ...
                    # file_path = "C:\\Users\\ezhao\\PycharmProjects\\MARL_Test\\output.txt"
                    # try:
                    #     with open(file_path, 'w') as file:
                    #         # Do something with the file, like reading its contents
                    #         file.write("action_dist_type\n")
                    #         file.write(str(type(action_dist)))
                    #         file.write("\n")
                    #         file.write("actions-------------------\n")
                    #         file.write(str(actions))
                    #         file.write("\n")
                    #         file.write("action dist -------------------------------\n-")
                    #         file.write(str(isinstance(action_dist,TorchMultiCategorical)))
                    #
